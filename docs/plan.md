...
| 10 | 10.3 | **preview ↔ chat 双方向フォーカス切替** | ユーザー指示 (2026-06-08) で 10.2 直後に繰り上げ。preview と chat ペインをキーボードで往復できるようにする。bind 案: `Cmd+J` (chat に focus) / `Cmd+K` (preview に focus)、もしくは `Cmd+/` で toggle。Cmd+1..9 のタブ切替と衝突しない bind を選ぶ。`egui::Memory::request_focus(id)` でフォーカス移動 | 10.2 | — |
