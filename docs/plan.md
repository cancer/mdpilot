...
| 7 | 7.8 | テーマ追従 | egui 0.34 のデフォルト `ThemePreference::System` で OS 追従が組み込み済み。eframe (`wgpu_integration.rs:276`) が `event_loop.system_theme()` を `egui_winit::State::new` に渡し、winit が `WindowEvent::ThemeChanged` をハンドリングするのでランタイム切替も追従。`egui_commonmark_backend::misc.rs:86` で syntect テーマは `ui.style().visuals.dark_mode` 基準に自動分岐するため、`src/preview/render.rs` で設定した `SYNTAX_THEME_DARK` (`base16-ocean.dark`) と `SYNTAX_THEME_LIGHT` (`InspiredGitHub`) がそのまま OS テーマと連動。`main.rs` でも `App` でも `ctx.set_theme(...)` を呼んでいないので explicit override 無し。実装変更なし、docs だけ更新 | 4.3 | ✓ |
...
| 9 | 9.1 | F-21 リンク・画像（相対パス含む）の解決の精緻化 | (A) `egui_commonmark` features に `fetch` を追加。`egui_extras/http` 経由で `ehttp` が組み込まれ、HTTPS 画像が描画される。MVP では warning icon のみだったケース（`docs/preview.md` §6）が解消。実機 smoke で httpbin の png ロードを確認。(B) `src/preview/watcher.rs::IMAGE_EXTENSIONS` 定数（`png/jpg/jpeg/gif/webp/svg`）+ `is_image_path` pure helper を追加。`classify_project_event` を `.md` だけでなく `is_image_path` も emit するよう拡張。App::drain_project_events に分岐を追加：image なら `crate::preview::image::to_file_uri(path)` で `file://<abs>` URI を組み立てて `ctx.forget_image(uri)` + `request_repaint`、`.md` は既存の auto-follow 経路。`Removed` の image branch も同じく forget_image で warning icon に戻す。`image::to_file_uri` を `pub(crate)` に格上げ。unit test 4 件追加（image 拡張子の認識/拒否、project_classifier の image-only + mixed md/image）。HTTPS 画像の自動リロードはローカルファイル監視対象外なのでスコープ外 | 4.4, 4.5 | ✓ |
