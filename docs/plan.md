...
| 10 | 10.0 | Phase 10 設計合意 (2026-06-08 ユーザー対話で確定) | キーボードフル操作 + エディタの vim 化 (preview のみ) に踏み切る判断。設計詳細は §3 Phase 10 を参照。本セルは合意の根拠ログ | — | ✓ |
| 10 | 10.1 | vim engine 共通モジュール | `src/vim/mod.rs` に modal state machine (Normal/Insert/Visual) + cursor + 編集 op + undo stack。`apply_key(state, key) -> Action` の pure テスト可能な層を中心に組む。preview への組み込み専用で設計 (chat 用途は外した) | — | ✓ |
| 10 | 10.2 | preview を編集可能化（基盤） | preview ペインを `egui::TextEdit::multiline` に置換、左に行番号 painter。常時編集可能、デフォルト Normal モード。`PreviewState::Loaded` の `LoadedDocument.text` を buffer として使う | 9.20, 10.1 | ✓ |
| 10 | 10.3 | **preview ↔ chat 双方向フォーカス切替** | 当初の bind は `Cmd+J`/`Cmd+K`、最終的にユーザー指示で `Cmd+Shift+H`/`Cmd+Shift+L` (Phase 10.20 / 10.20.1)。macOS の `Cmd+H` 衝突を避けるため Shift を噛ませる。`Cmd+1..9` (タブ切替) とは別系統 | 10.2 | ✓ |
| 10 | 10.4 | preview に keystroke save | Insert 中 / Normal 中いずれもキーを押すたびに `fs::write` で disk 反映。`Tab::last_written_hash` を更新、自分の write の ProjectWatcher エコーは hash 一致で drop | 10.2 | ✓ |
| 10 | 10.5 | Claude 競合検出 + 解決バナー | watcher イベントで disk hash != last_written_hash → 競合。preview ペイン上部にバナー: 「ディスクを読む (buffer 破棄して reload) / buffer を保つ (次回保存で上書き) / diff (MVP 後)」 | 10.4 | ✓ |
| 10 | 10.6 | vim 検索 (`/` `n` `N`) | buffer 内の string 検索。Normal モードで `/` → 入力欄 → Enter で検索、n / N で次/前のマッチへジャンプ。一致箇所をハイライト | 10.1 | ✓ |
| 10 | 10.7 | 編集中 auto-follow の確認モーダル | Claude が別 .md を書いた時、編集中なら「開きますか？ (Yes: 切替 / No: 留まる)」モーダル。No なら background tab で開くなどの選択肢は MVP 後 | 6.3, 10.2 | ✓ |
| 10 | 10.8 | ~~chat 入力欄に vim binding~~ | **削除** (2026-06-08 ユーザー判断): 「chatのvim化はいい」=不要。chat 入力欄は egui の素の TextEdit のまま、Plain Enter で送信 + Shift+Enter で改行という現状の挙動を維持 | — | ✗ |
...
| 10 | 10.11 | VS Code 風 fuzzy file finder (`Cmd+P`) | プロジェクト内 `.md` (`.mdx` 含む) を `nucleo-matcher` でスコアリング。modal は中央上寄せ、入力欄 + 結果リスト。`↑/↓` で選択、`Enter` で open、`Esc` で close。`Cmd+N`/`Cmd+P` でも上下選択可。実装は `src/ui/fuzzy_finder.rs` | 9.X.4 | ✓ |
| 10 | 10.12 | `Cmd+Shift+[` / `Cmd+Shift+]` でタブサイクル | 開いてるタブを左右にループ。`Cmd+1..9` の直接指定とは別系統で「次へ/前へ」だけ提供 | 9.5 | ✓ |
| 10 | 10.13 | セッション resume 時に chat 履歴 replay | 履歴ピッカーから resume したタブで `~/.claude/projects/<encoded>/<session_id>.jsonl` を読み、`User`/`Assistant` メッセージを ChatHistory に再構成。これで再起動後も会話が見える | 10.10, 9.X.1 | ✓ |
| 10 | 10.14 | working 中断 + Send/Abort toggle | claude CLI 2.1 に protocol interrupt がない問題は kill + `--resume` で迂回 (Phase 3.6 の方針を覆して実装)。`in_flight` フラグで送信ボタンと中断ボタンを排他切替、`Esc`(chat focus) でも abort 可。abort 時は `ResultError("aborted_by_user")` を system message として履歴に残す | 3.6 (再開), 9.X.1 | ✓ |
| 10 | 10.15 | chat-pane 選択時に「→チャット」 bubble を抑止 | LabelSelectionState は app-global なので、preview ペイン rect 外で発生した選択は bubble の対象にしない | 10.6 | ✓ |
| 10 | 10.16 | preview スクロール 3 件のバグ修正 | (a) tree open 時に wheel が tree に奪われる → `Panel::left` で独立 scroll region に分離。(b) vim カーソル移動で画面外に行っても追従しない → `EditorState.scroll_to_cursor` を `Action.cursor_moved` で立て、render で `scroll_to_me(Center)`。(c) 末尾改行追加で追従しない → 同フラグで対応 | 10.2 | ✓ |
| 10 | 10.17 | Normal-mode `Y` で `@<path>` を chat input に挿入 | 選択なし `Y` は preview の現在ファイル相対パスを `@<rel>` 形式で chat input に append、focus も chat へ。Visual `Y` は既存の引用ブロック挙動を維持 | 10.6 | ✓ |
| 10 | 10.18 | User メッセージに tint Frame + 控えめ本文色 | User と Assistant の見出しが同コントラストで視認性が悪かったので、User 側だけ subtle bg + 低コントラスト本文色 (gray 170/90) に変える。Assistant は装飾なし高コントラストのまま | — | ✓ |
| 10 | 10.19 | claude stderr のエラーを chat に出す | `ChatEvent::Stderr`/`SystemMessage::StderrError` を追加。`pipe_stderr_to_channel` が `error/fatal/panic/unauthor` を含む行だけ chat に送る。`ANTHROPIC_API_KEY` 未設定等が UI に出ないサイレント死を解消 | 2.4 | ✓ |
| 10 | 10.20 | ペインフォーカス切替の bind 変更 | `Cmd+J`/`Cmd+K` (上下軸) → `Cmd+Shift+H`/`Cmd+Shift+L` (左右軸)。preview/chat の横並び物理配置と vim の h/l 方向が一致する。Shift を噛ませて macOS `Cmd+H` の「アプリを隠す」と衝突回避 | 10.3 | ✓ |
| 10 | 10.21 | Enter 送信時に TextEdit へ改行が漏れる回帰修正 | `extract_send_enter` を `ui.add_sized(textedit)` の **前** に呼ぶ。pre-consume で TextEdit がそもそも Enter を見ないようにする。focus 判定は `ctx.memory()` で行う | 10.14 | ✓ |
| 10 | 10.22 | fuzzy finder / session picker 中の vim 引き込み防止 | `feed_vim_events` で `fuzzy_finder.is_some() \|\| session_picker.is_some()` のとき早期 return。modal の TextEdit にキーが届くようにする | 10.11 | ✓ |
| 10 | 10.23 | Normal-mode `Y` で `@<path>\n` 末尾改行 + カーソル末尾移動 | `TextEditState::load → set_char_range → store` で chat input の cursor を `@path` の直下行頭に移動。続けて本文を即書き始められる | 10.17 | ✓ |
| 10 | 10.24 | Assistant 見出しの削除 | 10.18 で User 見出しを外したのに合わせて Assistant 見出しも削除。User の tint Frame そのものが speaker 区別になっているので冗長 | 10.18 | ✓ |
| 10 | 10.25 | 送信ボタン撤廃 + mid-stream 送信許可 | 送信ボタンを UI から削除 (Enter のみ)。中断ボタンは `in_flight` 時だけ右側に出す。`in_flight` でも Enter 送信可能、内部的には「現 turn を abort してから新 turn を開始」する形で実装。`aborted_by_user` system marker が turn1/turn2 の間に挟まる | 10.14, 10.21 | ✓ |
| 10 | 10.26 | 送信時に chat ペインを末尾までスクロール | `ChatHistory.scroll_to_bottom_pending` one-shot フラグを立て、ScrollArea closure 内で `scroll_to_cursor(Align::Max)`。`stick_to_bottom` だけだと過去履歴閲覧中に送ったメッセージが画面外に残るのを回避 | 10.25 | ✓ |
| 10 | 10.27 | `.mdx` を markdown 拡張子として認識 | `MARKDOWN_EXTENSIONS` に `mdx` 追加。file tree / fuzzy finder / watcher 全部で `.mdx` を拾う。JSX 部分は egui_commonmark で素通りするが本文 markdown 構造は読める | 9.X.4 | ✓ |
| 10 | 10.28 | tool input delta + tool_result イベントの parse | `ChatEvent::ToolInputDelta` (`stream_event/content_block_delta/input_json_delta`) + `ChatEvent::ToolResult` (`"user"` イベント内の `tool_result` 配列を展開) を追加。`parse_event` を `parse_events: Vec<ChatEvent>` でラップし pipe_stdout_to_channel が複数 event を 1 行から送れるように。tool 引数の `{}` 表示と `(出力待ち…)` 永続が解消 | 3.3 | ✓ |
| 10 | 10.29 | 大ファイルの per-frame 描画コスト削減 (行 Galley キャッシュ) | 2500 行 md でキー入力ごとにラグが出ていた問題への対処。`EditorState.body_galley_cache: HashMap<u64, Arc<Galley>>` を追加し、`(行テキスト, theme, search match ranges, wrap width)` をキーに引いて hit 時は `LayoutJob` 構築と `layout_job` 呼び出しを完全にスキップ。syntect の `highlight_line` は per-line 状態を壊さないため毎行常時呼ぶ (保守的判断)。フレーム単位で entries を rotate して memory bound | 10.2 | ✓ |
...
**適用対象は preview ペインのみ**（2026-06-08 ユーザー判断で chat 入力欄は
対象外、素の TextEdit のまま）。
...
- **preview ↔ chat 双方向フォーカス切替** (10.3 / ユーザー強い要望):
  `Cmd+J`/`Cmd+K` または `Cmd+/` で往復。Cmd+1..9 (タブ切替) と衝突しない bind を選ぶ
- file tree: j/k 移動、Enter 開く、Space 展開、Esc preview に戻す
- 履歴ピッカー: j/k 移動、Enter resume、Esc 閉じる

#### 実装サブフェーズ（順序: 2026-06-08 ユーザー指示でフォーカス切替を 10.3 に繰り上げ）

10.1 vim engine → 10.2 **preview 編集化** → 10.3 **ペイン切替 (preview ↔ chat)** →
10.4 keystroke save → 10.5 Claude 競合 → 10.6 検索 → 10.7 auto-follow モーダル →
~~10.8 chat vim 適用~~（削除） → 10.9 tree keynav → 10.10 履歴 keynav

各サブフェーズで実機検証 + commit。**10.1-10.5 がコア**（preview を編集できて、
preview ↔ chat をキーボードで往復できて、編集が disk に反映され、Claude 競合が
解決できる）。10.6 以降は段階的に積む。**10.8 (chat への vim 適用) は削除**
（ユーザー判断で不要）。
