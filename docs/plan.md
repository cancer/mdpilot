...
| 1 | 1.1 | レイアウト状態とペイン分割（F-01 前半） | `LayoutState`, `egui::SidePanel`/分割比率の保持 | 0.5.2 | — |
| 1 | 1.2 | 境界リサイズハンドル（F-01 後半） | マウスドラッグで比率変更、ダブルクリックで 1:1 リセット、最小幅 240px | 1.1 | — |
| 1 | 1.3 | プレビュー/チャットのプレースホルダ描画 | 「空ペイン」表示、ウィンドウタイトルは固定文字列 | 1.1 | — |
| 2 | 2.1 | claude 子プロセスの起動 | `std::process::Command` で `claude` を spawn、`Stdio::piped()` で stdin/stdout/stderr | 0.3 | — |
