...
- 起動直後は最上端から表示
- **MVP**：ファイル再読込時はスクロールをリセットし、最上端から表示する（同一パス再読込・別ファイル切替のどちらも同じ挙動）
- MVP 後（`docs/plan.md` Phase 9.14）：同一パスの再読込時のみ「再読込前の最上端行に近い位置」へ復元する行番号ベース保持を実装する。`egui_commonmark` 0.23 は source-line → rendered Y マッピングを公開していないため、block-level 分割再レンダリングまたは fork が必要で、MVP スコープ外と判断（2026-06-03 ユーザー判断）
