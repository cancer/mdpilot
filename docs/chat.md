...
| `Esc`（応答中） | 中断ボタン相当（claude CLI 2.1 では未対応、§10 参照） |
...
### 10.1 中断機構: MVP では未対応（Phase 3.6 で確定）

mdpilot MVP では **中断ボタンを永続的に disabled 表示** し、tooltip で理由を説明する。これは claude CLI 2.1.132 が headless mode（`--print --input-format=stream-json --output-format=stream-json`）でリアルタイム中断手段を提供していないためで、フォールバック実装はすべて副作用が大きいため MVP では採用しない。
...
`{"type":"interrupt"}` を stdin に送る形式は issue で要求されたが未実装。issue 内 Alternative Solutions の検証結果:

| 候補 | 制限 | mdpilot での評価 |
|---|---|---|
| `{"type":"interrupt"}` stdin メッセージ | claude CLI 2.1 で未実装 | 採用不可 |
| Agent SDK `interrupt()` メソッド | Python / TypeScript SDK のみ。API 課金前提（Claude subscription 経由では使えない）| mdpilot は CLI 直接 spawn のため使えない |
| `SIGINT` / `CTRL_BREAK` | headless mode ではプロセスを完全に kill する | セッション喪失。再生成と組み合わせれば動くが MVP には複雑すぎる |
| Kill + `--continue` 再生成 | 遅い、進行中の tool state を失う | MVP では受け入れがたい UX |
| 実行中に stdin へ書込 | 次の turn 境界まで queue される、リアルタイム中断ではない | text-only ターンに効かない |

claude CLI 側で `{"type":"interrupt"}` がサポートされた時点で Phase 9 として有効化する。再有効化のフックを `src/chat/view.rs` の 中断 button 周辺に残してある（ボタン自体は描画されるが `add_enabled(false, ...)` で常時無効）。
...
| ツール `tool_result` イベントの正確なスキーマ | Phase 2 で `tool_use` は確認、`tool_result` は未取得 |
