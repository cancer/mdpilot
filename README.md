...
mdpilot は **別途インストールされた `claude` CLI** を子プロセスとして spawn します。
mdpilot 自体は claude CLI を同梱しないため、利用前にユーザー自身でインストールが必要です。

```sh
npm install -g @anthropic-ai/claude-code
# インストール手順は https://docs.claude.com/claude-code を参照
```

ターミナルで `which claude` が成功することを確認してから mdpilot を起動してください。
mdpilot を Finder / Dock から起動する場合、login shell 経由で `PATH` を解決するので
shell の rc に claude のインストール先が追加されていれば動作します。
