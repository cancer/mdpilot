...
- 完了: Phase 0.1〜0.4, 0.5.2/0.5.3, 仕様改訂, 2.0〜2.7, 1.1〜1.4, 3.1〜3.7（Phase 3 完了）, 4.1〜4.5, 5.1, 5.2
...
- 次の着手対象: **Phase 5.3（監視エラーのステータス表示）**。監視開始失敗をステータスバー/トーストに出す、`Cmd+R`/`Ctrl+R` で手動再読込（`docs/preview.md` §7）
- Phase 5.2 の残課題: `MDPILOT_DEBUG_SCREENSHOT` で 起動 30 フレーム待つ間にバックグラウンドプロセスがハングする問題が再発（CPU 100%+ で frame loop 進まず）。これは Phase 5.2 由来ではなく `MDPILOT_PREVIEW_FILE` 無しの bare app でも再現するので、window focus 等の環境要因（CLAUDE.md 旧 Phase 3.6 note と同じ）。Phase 5.2 の F-08 e2e（編集→100ms 後に自動再描画、削除→「見つかりません」、再作成→自動復元）は実機インタラクティブ検証が必要
