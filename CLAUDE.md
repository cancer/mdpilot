...
- 完了: Phase 0.1〜0.4, 0.5.2/0.5.3, 仕様改訂, 2.0〜2.7, 1.1〜1.4, 3.1〜3.4
- 次の着手対象: **Phase 3.5（実機 claude 統合：起動時 spawn / mpsc 配線 / 送信ボタンフロー / Enter+Shift+Enter 判定 / IME 実機確認）**
...
| スレッド | 役割 |
|---|---|
| メイン | eframe イベントループ、UI 描画、claude stdin への書込み |
| BG | claude stdout 読込 → `serde_json::Value` パース → `ChatEvent` → mpsc に流す（Phase 3.5 で App に配線） |
| BG | claude stderr 読込 → `tracing::warn`（`target = "claude::stderr"`） |
| BG（Phase 5+） | `notify` ファイル監視 |

### `src/chat/` モジュール

- `session.rs` — `ChatSession`（child process owner）、`SpawnOptions`、`build_args` (pure fn)、`send_user_message`、`pipe_lines_to_tracing`、`Drop` は **stdin drop → SIGTERM → 500ms 待機 → SIGKILL**（Unix）/ Windows は `Child::kill()`。`libc` は `[target.'cfg(unix)'.dependencies]`
- `stream.rs` — `ChatEvent` enum + `parse_event(&Value)` + `pipe_stdout_to_channel<R: BufRead>`。**typed `serde::Deserialize` は使わない**（`system/init` に 20+ フィールドあり、claude バージョンで増減するため `Value::get(...)` 抽出のみ）
- `history.rs` — `ChatHistory` (Vec<ChatMessage> + input + pending_send 予定)、`apply(ChatEvent)` で TextDelta/AssistantMessage/ToolUse/ApiRetry/Result を内部状態にマップ
- `session_store.rs` — `<data_dir>/sessions.json` の atomic write（tmp + rename）。プロジェクトルートごとに session-id を永続化、chrono タイムスタンプ付き
- `view.rs` — chat pane の描画（メッセージ履歴 + 入力欄 + 送信/中断ボタン + tool collapsible block）
...
- 中断ボタンの実装方式（SIGINT / `--max-turns` / 別 RPC）— Phase 3.6
