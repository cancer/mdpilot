...
- Phase 5.2/5.3 の残課題: `MDPILOT_DEBUG_SCREENSHOT` で 起動 30 フレーム待つ間にバックグラウンドプロセスがハングする問題が再発（CPU 100%+ で frame loop 進まず）。これは Phase 5.2/5.3 由来ではなく `MDPILOT_PREVIEW_FILE` 無しの bare app でも再現するので、window focus 等の環境要因（CLAUDE.md 旧 Phase 3.6 note と同じ）。F-08 e2e（編集→100ms 後に自動再描画、削除→「見つかりません」、再作成→Cmd+R で復元）+ Cmd+R / Ctrl+R 動作 + watcher_error バナー表示は実機インタラクティブ検証が必要
