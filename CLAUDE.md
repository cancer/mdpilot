...
- 完了: Phase 0.1〜0.4, 0.5.2/0.5.3, 仕様改訂, 2.0〜2.7, 1.1〜1.4, 3.1〜3.7（Phase 3 完了）, 4.1〜4.5, 5.1〜5.3（Phase 5 完了）, 6.1, 6.2
...
- 次の着手対象: **Phase 6.3（自動追従ロジック・F-09 案 A）**。「現在表示中以外の `.md` 書き換え」で対象切替、200ms デバウンス（`docs/claude-integration.md` 6.2、`docs/plan.md` タスク 6.3）
...
- `watcher.rs` — 2 種類の watcher が同居:
  ...
  - **`ProjectWatcher`**（Phase 6.2）: プロジェクトルート `Recursive` watch。`EXCLUDED_DIRS` 定数（`.git` / `node_modules` / `target` / `dist` / `build` / `.next` / `.svelte-kit` / `.venv` / `__pycache__`）+ `is_excluded_dir` / `is_markdown_path` / `is_in_excluded_subtree(path, root)`（intermediate components のみ）+ `classify_project_event(event, root)`（pure、`.md` 以外と excluded 配下を drop）。`ProjectWatcher::start(root, events_tx, wake_ui)` で root を所有・watch を即時アタッチ（spec 上ルートは mdpilot 起動中不変）。App 配線は `app.rs::start_project_watcher` / `drain_project_events`（Phase 6.2 は tracing::info のみ、Phase 6.3 で auto-follow に置換）。両 watcher とも `notify` の dispatcher thread に乗るので追加の `std::thread` は spawn しない
