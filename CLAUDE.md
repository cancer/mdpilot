...
- 完了: Phase 0.1〜0.4, 0.5.2/0.5.3, 仕様改訂, 2.0〜2.7, 1.1〜1.4, 3.1〜3.7（Phase 3 完了）, 4.1〜4.2
- 次の着手対象: **Phase 4.3（syntect シンタックスハイライト統合）**。`egui_commonmark` の `better_syntax_highlighting` feature を有効化、ダーク/ライト 2 テーマと 1MB 超ブロックフォールバック方針を決定
...
- `render.rs` — `PreviewState { status, cache }` + `show(ui, &mut state)`。`PreviewStatus` は `Empty / Loaded(LoadedDocument) / Failed { path_label, error }`。Loaded は `ScrollArea` 内で `CommonMarkViewer::new().show(ui, &mut cache, &text)`、Large は警告バナー、Failed は色付き見出し + パス + 詳細。`error_headline` / `error_detail` / `size_warning` は pure fn で unit test 済み。`set_document` 時に `CommonMarkCache::default()` で cache をクリアし古いシンタックスハイライト状態を漏らさない
