...
- 完了: Phase 0.1〜0.4, 0.5.2/0.5.3, 仕様改訂, 2.0〜2.7, 1.1〜1.4, 3.1〜3.7（Phase 3 完了）, 4.1〜4.5, 5.1, 5.2, 5.3（Phase 5 完了）
...
- 次の着手対象: **Phase 6.1（プロジェクトルート解決）**。起動引数（`mdpilot <dir>` / `<file>` / 引数なし）から root を決定（`docs/claude-integration.md` 2 章、`docs/plan.md` タスク 6.1）
- screenshot ハングの調査結果（2026-06-03）: `--enable-dev-tools` 化後に 7 連続 run（foreground / background / pipe あり）すべて成功、frame 1→30 で約 500ms。Phase 5.2/5.3 時の hang を再現できず、根本原因は未特定。仮説として「`cfg(debug_assertions)` 時代は debug build を起動するたび helper が自動 trigger され orphan プロセスが溜まっていた可能性」があり、`--enable-dev-tools` 明示 opt-in が結果的に mitigation になっている。`src/app.rs::DebugScreenshot` に (a) `mdpilot::devtools` target の tracing instrumentation (frame tick + request log) (b) `WATCHDOG_FRAMES_AFTER_REQUEST = 300`（約 5 秒）の watchdog を追加。次に hang したら trace log で frame の進み方が分かるし、watchdog が orphan 化を防ぐ
...
- Phase 3.6 の残課題: disabled ボタン + on_disabled_hover_text の見た目はコード上は egui 0.34 の正しい API（clippy / test green）。次に「中断」ボタンが描画されるシナリオで実機目視確認すれば閉じられる（screenshot 問題は 2026-06-03 に上で記述した tracing + watchdog で再診断できる状態）
...
- `watcher.rs` — `FileWatcher` + `FileWatchEvent { Changed/Removed/Error }` + `classify_event(notify::Event) -> Vec<FileWatchEvent>`（pure）+ `paths_match(a, b) -> bool`（exact equality → fs::canonicalize fallback、macOS FSEvents の `/private/var/...` 正規化対応）+ `reload_decision(deadline, now) -> ReloadStep::{Idle, Wait{remaining}, Fire}`（pure、debounce 判定）+ `RELOAD_DEBOUNCE = 100ms` 定数。`FileWatcher::start(events_tx, wake_ui)` で `notify::recommended_watcher` を作成、コールバックで `Modify/Create→Changed`、`Remove→Removed`、`Access/Any/Other` は drop。`notify` の dispatcher thread に乗るので追加の `std::thread` は spawn しない。`watch/unwatch/unwatch_all/watched_paths` で App が path 管理。App 配線は `app.rs` の `sync_watch_target` / `drain_watch_events` / `poll_pending_reload` / `reload_current` / `handle_removed`（Phase 5.2）
