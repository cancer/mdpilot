...
- 完了: Phase 0.1〜0.4, 0.5.2/0.5.3, 仕様改訂, 2.0〜2.7, 1.1〜1.4, 3.1〜3.7（Phase 3 完了）, 4.1〜4.4
- 次の着手対象: **Phase 4.5（画像・相対パス解決）**。`preview.md` §6 の通り、ローカル相対/絶対パス画像を表示、HTTP/HTTPS は MVP 非対応（警告アイコンのみ）。egui_commonmark の image hooks は spike-report §0.5.2 で挙動確認済み（不在ファイルは警告アイコンのみ、alt は出ない）
